...
int disallow_cipher(int argc, char** argv, ostream& os)
{
    gConfig.set("GSM.Cipher", 0);
    return SUCCESS;
}

int allow_cipher(int argc, char** argv, ostream& os)
{
...
void Parser::addCommands()
{
	...
	addCommand("disallow_cipher", disallow_cipher, "-- disable encryption globally (recommended for testing only, use per-imsi settings instead)");
	addCommand("allow_cipher", allow_cipher, "-- enable encryption globally (if it was disabled for testing)");
